...
const Results = props => {
  ...
  const handleTicketsSuccessResponse = (responseObject) => {
    if (
      ...
      && (typeof actionsCol === 'function')
        ...
        try {
          ...
          responseObject.payload[key]['status'] = getTicketStatus(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(ticketsServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleTicketsSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "tickets/getUserTicketsServerSide", {
    method: 'POST',
    headers: {
