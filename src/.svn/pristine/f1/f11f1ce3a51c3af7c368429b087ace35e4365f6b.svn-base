...
const UsersAdd = () => {
  ...
  const handleSubmit = async event => {
    ...
    dispatch(addUsers(formState.values));
  }
                ...
                <Grid item xs={6} sm={3}>
                  {(rolesState.rolesList) ?
                    <Autocomplete
                      ...
                      size="small"
                      renderInput={(params) => <TextField {...params} label="Select Roles" variant="outlined" error={hasError('roles_ids')} helperText={hasError('roles_ids') ? formState.errors.roles_ids[0] : null} />}
                    />
                    : ''}
                </Grid>
                <Grid item xs={6} sm={3}>
