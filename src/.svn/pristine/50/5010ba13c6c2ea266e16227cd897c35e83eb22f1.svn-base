...
const AuditErrorCategoryList = () => {
  ...
  const [auditErrorCategoryId, setAuditErrorCategoryId] = useState('');
  ...
  useEffect(() => {
    if (auditErrorCategoryState.showViewPage) {
      router.history.push('/activity-audit/error-category/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [auditErrorCategoryState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteAuditErrorCategory(auditErrorCategoryId, session.current_page_permissions.object_id));
    setRefershDataTable(true);
  }

  const showDeleteModal = (id) => {
    setAuditErrorCategoryId(id)
    setOpenDeleteModel(true)
  }

  const hideDeleteModel = () => {
    setAuditErrorCategoryId('')
  }

  const updateRecord = (id) => {
    dispatch(getAuditErrorCategoryById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getAuditErrorCategoryById(id, 'view'))
  }
  ...
  return (
    <Page
      ...
      title="Audit Error Category List"
    >
      <Header />
      <Results
...
export default AuditErrorCategoryList;
