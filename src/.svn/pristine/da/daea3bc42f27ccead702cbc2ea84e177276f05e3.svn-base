...
const Results = props => {
  ...
  const handleDesignationSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(designationServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleDesignationSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "designation/getAllServerSide", {
    method: 'POST',
    headers: {
