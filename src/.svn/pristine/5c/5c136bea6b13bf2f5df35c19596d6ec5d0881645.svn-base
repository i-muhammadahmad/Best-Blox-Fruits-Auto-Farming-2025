...
import {
  auditFormCategoryServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = (props) => {
  ...
  //handle custom response for audit form categiry
  const handleAuditFromCatSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(auditFormCategoryServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleAuditFromCatSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "auditFormCategory/getAllServerSide", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              <CustomDataGrid
                ...
                gridName="AuditFormCategoryGrid"
              />
            </div>
          </PerfectScrollbar>
        </CardContent>
