...
const TimeZonesList = () => {
  ...
  const [timeZonesId, setTimeZonesId] = useState('');
  ...
  useEffect(() => {
    if (timeZonesState.showUpdateForm) {
      router.history.push('/timeZones/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [timeZonesState.showUpdateForm]);
  ...
  const deleteRecord = async () => {
    await dispatch(deleteTimeZones(timeZonesId, session.current_page_permissions.object_id));
  ...
  const updateRecord = (id) => {
    dispatch(getTimeZonesById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getTimeZonesById(id, 'view'))
  }
