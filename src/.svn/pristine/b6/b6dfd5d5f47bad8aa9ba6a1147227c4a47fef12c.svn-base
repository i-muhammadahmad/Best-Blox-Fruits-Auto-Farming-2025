  ...
  const getActions = value => {
    return (
      <div className={'actionClass'} style={{ textAlign: 'center', whiteSpace: 'nowrap' }}>
        { (AccessRights(client_invoice_per[0], 'edit', value.created_by) && value.is_deleted == 'n') ?
          <><StyledFab
            color="bprimary"
            aria-label="Edit"
            size="small"
        ...
        { (AccessRights(client_invoice_per[0], 'view', value.created_by)) ?
          <><StyledFab
            color="bwarning"
            aria-label="View"
            size="small"
        ...
        { (AccessRights(client_invoice_per[0], 'view', value.created_by) && !isEmpty(value.attachment_url))? 
          <><StyledFab
            color="bgrey"
            aria-label="View"
            size="small"
        ...
        {(AccessRights(client_invoice_per[0], 'delete', value.created_by) && value.is_deleted == 'n') ?
          <StyledFab
            color="bdanger"
            aria-label="edit"
            size="small"
