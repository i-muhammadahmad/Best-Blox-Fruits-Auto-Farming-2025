...
import {
	...
	activityAllParentCategoryDropdownListFetch
} from 'actions';
import {
	Card,
	CardHeader,
...
const ActivityLogAdd = () => {
	...
	useEffect(() => {
		dispatch(
			activityAllParentCategoryDropdownListFetch(
				session.user.employee.client_id,
	...
	useEffect(() => {
		if (formState.values.chk_manual_time === true) {
	...
	return (
		<Page className={classes.root} title="Add Activity Log">
			<Header />
									...
									{activitySetupState.activitySetupByRoleList ? (
										<>
											<Autocomplete
												...
												disabled={isEmpty(selectedCategoryId)}
