...
const Header = props => {
  ...
  const deleteRecord = async () => {
    await dispatch(deleteActivityAccess(activityAccessState.activityAccessRecord.id, session.current_page_permissions.object_id));
    closeView();
  }
  ...
  const updateRecord = () => {
    dispatch(getActivityAccessById(activityAccessState.activityAccessRecord.id, 'update'))
  }
      ...
      >
        ...
        <Grid item>
          <ViewActionButtons  
            ...
            currentRecord={activityAccessState.activityAccessRecord}
          />
        </Grid>
      </Grid>   
      <DeleteAlert
