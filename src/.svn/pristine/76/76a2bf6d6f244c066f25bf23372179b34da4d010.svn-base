...
const CheckpointDetails = props => {
  ...
  const classes = useStyles(); 
	...
	const removeSupportingDoc = (chk_id, detail_id, image_id) => {
		...
		if(!isEmpty(checkpoints_docs_arr[image_id]['id'])){
			
			setDeletedFiles(deletedFiles => ({
				...deletedFiles,
				[checkpoints_docs_arr[image_id]['id']]: checkpoints_docs_arr[image_id]['id']
			}));
		}
	...
	const removeCheckpointDetail = async (chk_id, detail_id) => {
		...
		if(!isEmpty(checkpoints_deatil_arr[detail_id]['id'])){
			
			setDeletedDetails(deletedFiles => ({
				...deletedFiles,
				[checkpoints_deatil_arr[detail_id]['id']]: checkpoints_deatil_arr[detail_id]['id']
			}));
		}
