...
const OfficesList = () => {
  ...
  useEffect(() => {
    if (officesState.showUpdateForm) {
      router.history.push('/offices/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const deleteRecord = async () => {
    await dispatch(deleteOffices(officesId, session.current_page_permissions.object_id));
  ...
  const updateRecord = (id) => {
    dispatch(getOfficesById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getOfficesById(id, 'view'))
  }
  ...
  const getFlagAvator = value => {
    return (
      <div className={'actionClass'} style={{ textAlign: 'center', whiteSpace: 'nowrap' }}>
        {(value.flag_image) ?
          <Avatar alt="Avator" src={API_URL + value.flag_image} className={classes.large} />
          :
          ''
        }
      </div>
