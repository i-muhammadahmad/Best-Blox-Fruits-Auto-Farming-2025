...
import {
  complianceReportServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  const { className, refershDataTable, setRefershDataTable, extraFiltersState, getCoursesStatus, actionsCol,  ...rest } = props;

  const dispatch = useDispatch();

  //handle custom response for quizes
  const handleComplianceReportSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['quiz_status'] = getCoursesStatus(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(complianceReportServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleComplianceReportSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "compliance/detailReport", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              <CustomDataGrid
                columns={Columns}
                dataSource={
                  new ServerSideRequest(server_side_request, '', call_backs, extraFiltersState.values)
                }
                gridName="ComplianceReportGrid"
              />
            </div>
          </PerfectScrollbar>
        </CardContent>
