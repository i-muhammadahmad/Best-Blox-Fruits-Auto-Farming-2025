...
const Results = props => {
  ...
  const handleHolidaySuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['holiday_clients'] = getHolidayClients(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(holidayServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleHolidaySuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "holiday/getAllServerSide", {
    method: 'POST',
    headers: {
