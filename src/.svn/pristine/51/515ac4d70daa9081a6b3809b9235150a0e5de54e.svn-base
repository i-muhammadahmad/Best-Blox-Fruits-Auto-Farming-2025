...
const Results = props => {
  ...
  const handleCompanySuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['company_image_icon'] = getCompanyAvator(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(companyServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleCompanySuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "company/getAllServerSide", {
    method: 'POST',
    headers: {
