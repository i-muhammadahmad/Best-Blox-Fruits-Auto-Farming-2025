...
import {
  auditFormInfractionServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  const handleAuditFromInfractionSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(auditFormInfractionServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleAuditFromInfractionSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "audit_form_infractions/getAllServerSide", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              ...
              <CustomDataGrid
                ...
                gridName="AuditFormInfractionGrid"
              />
            </div>
          </PerfectScrollbar>
        </CardContent>
