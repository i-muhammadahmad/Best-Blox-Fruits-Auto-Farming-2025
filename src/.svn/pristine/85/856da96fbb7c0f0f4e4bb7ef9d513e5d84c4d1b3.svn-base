...
const ActivityCategoryDropdown = props => {
	const {
		...
		setActivityCategoryValue,
		selectedId,
		selectedChangingId,
		activityCategoryOnChange,
		id,
		name,
		size,
		renderInput,
		...
		if (activityCategoryState.activityCategoriesDropdownList) {
			...
			if (!isEmpty(sCId)) {
				if (isArray(sCId)) {
					...
					setActivityCategoryValue(scategory);
				} else {
					...
					if (!isEmpty(selected_category)) {
						setActivityCategoryValue(selected_category);
					} else {
	...
	useEffect(() => {
		if (activityCategoryState.activityCategoriesDropdownList) {
			...
			if (
				attr.multiple === true &&
				!isEmpty(activity_category_list) &&
