...
const ApprovalProfilesAdd = () => {
  ...
  useEffect(() => {
    ...
    dispatch(employeesCustomDropdownListFetch([], [], session.current_page_permissions.object_id));
  ...
  const officeOnChange = (event, newValue) => {
    if (newValue) {
      ...
      if (newValue.id != 'all') {
        dispatch(employeesCustomDropdownListFetch([newValue.id], [], session.current_page_permissions.object_id));
      }
      else {
        dispatch(employeesCustomDropdownListFetch([], [], session.current_page_permissions.object_id));
      }
  ...
  const handleSubmit = async event => {
    ...
    if (formState.isValid) {
      ...
      dispatch(addApprovalProfiles(form_data));
    }
  }
  ...
  return (
    <Page
      ...
      title="Add Approval Profile"
    >
      <Header />
      <Card
        className={classes.projectDetails}
      >
        <CardHeader title="Add Approval Profile" />
        <CardContent>
          <form
...
export default ApprovalProfilesAdd;
