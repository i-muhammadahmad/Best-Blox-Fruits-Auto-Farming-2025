...
const MandatoryCoursesList = () => {
  ...
  useEffect(() => {
    if (quizSetupState.showViewPage) {
      router.history.push('/compliance/mandatory-courses/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const getActions = value => {
    return (
      <div className={'actionClass'} style={{ textAlign: 'center', whiteSpace: 'nowrap' }}>
        ...
        {(session.current_page_permissions.rights_delete == '1' && value.is_published === 'n') ?
          <StyledFab
            color="bdanger"
            aria-label="edit"
            size="small"
  ...
  return (
    ...
    >
      ...
      <DeleteAlert
        title="Quiz Delete"
        alertText="Are you sure, You want delete this Quiz?"
...
export default MandatoryCoursesList;
