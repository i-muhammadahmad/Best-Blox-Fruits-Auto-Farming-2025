...
const DesignationCategoryList = () => {
  ...
  const [designationCategoryId, setDesignationCategoryId] = useState('');
  ...
  useEffect(() => {
    if (designationCategoryState.showUpdateForm) {
      router.history.push('/designation-category/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [designationCategoryState.showUpdateForm]);
  ...
  const deleteRecord = async () => {
    await dispatch(deleteDesignationCategory(designationCategoryId, session.current_page_permissions.object_id));
    setRefershDataTable(true);
  }

  const showDeleteModal = (id) => {
    setDesignationCategoryId(id)
    setOpenDeleteModel(true)
  }

  const hideDeleteModel = () => {
    setDesignationCategoryId('')
  }

  const updateRecord = (id) => {
    dispatch(getDesignationCategoryById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getDesignationCategoryById(id, 'view'))
  }
  ...
  return (
    <Page
      ...
      title="Designation Category List"
    >
      <Header />
      <Results
