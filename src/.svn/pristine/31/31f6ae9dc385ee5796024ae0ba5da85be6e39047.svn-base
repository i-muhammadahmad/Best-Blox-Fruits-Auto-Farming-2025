...
const ObjectsAdd = () => {
  ...
  const handleSubmit = async event => {
    ...
    dispatch(addObjects(formState.values));
  }
          ...
          >
            <div className={classes.formGroup}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={6} sm={4}>
                  <TextField
                    error={hasError('object_code')}
                    fullWidth
                    helperText={hasError('object_code') ? formState.errors.object_code[0] : null}
                    label="Unique Object Code"
                    name="object_code"
                    onChange={handleChange}
                    value={formState.values.object_code || ''}
                    variant="outlined"
                    size="small"
                  />
                </Grid>
