...
const ClientHeadcountList = () => {
  ...
  const viewRecord = async (id, client_id) => {
    await dispatch(getShiftsByClient(client_id, session.current_page_permissions.object_id));
    await dispatch(designationListFetch(session.current_page_permissions.object_id));
