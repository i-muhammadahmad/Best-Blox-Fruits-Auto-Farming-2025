...
const Results = props => {
  ...
  const handleTimezoneSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(timeZonesServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleTimezoneSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "timezone/getAllServerSide", {
    method: 'POST',
    headers: {
