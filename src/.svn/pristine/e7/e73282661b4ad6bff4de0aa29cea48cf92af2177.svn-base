...
const Results = props => {
  ...
  const handleRolesSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(rolesServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleRolesSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "roles/getAllServerSide", {
    method: 'POST',
    headers: {
