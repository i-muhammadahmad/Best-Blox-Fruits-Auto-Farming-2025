...
import {
  deleteAuditFormSetup,
  getAuditFormSetupById,
} from 'actions';
...
const AuditFormSetupList = () => {
  ...
  const [auditFormSetupId, setAuditFormSetupId] = useState('');
  ...
  useEffect(() => {
    if (auditFormSetupState.showUpdateForm) {
      router.history.push('/audit-form-setup/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [auditFormSetupState.showUpdateForm]);

  useEffect(() => {
    if (auditFormSetupState.showViewPage) {
      router.history.push('/audit-form-setup/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [auditFormSetupState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteAuditFormSetup(auditFormSetupId, session.current_page_permissions.object_id));
    setRefershDataTable(true);
  }

  const showDeleteModal = (id) => {
    setAuditFormSetupId(id)
  ...
  const updateRecord = (id) => {
    dispatch(getAuditFormSetupById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getAuditFormSetupById(id, 'view'))
  }
  ...
  return (
    ...
    >
      ...
      />
      <DeleteAlert
        title="Audit Form Setup Delete"
        alertText="Are you sure, You want delete this Audit Form Setup?"
...
export default AuditFormSetupList;
