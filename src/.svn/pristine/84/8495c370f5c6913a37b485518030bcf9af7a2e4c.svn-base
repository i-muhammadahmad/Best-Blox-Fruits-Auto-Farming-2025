...
import {
  auditFormSetupServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  const handleAuditFromSetupSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(auditFormSetupServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleAuditFromSetupSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "audit_form_setup/getAllServerSide", {
    method: 'POST',
    headers: {
