...
const Results = props => {
  ...
  const handleCoachingsSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(auditFormServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleCoachingsSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "coaching/getAllServerSideForSupervisor", {
    method: 'POST',
    headers: {
