...
import {
  employeeAttritionServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  const handleEmployeeAttritionSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(employeeAttritionServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleEmployeeAttritionSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "employeeAttrition/getAllServerSide", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              ...
              <CustomDataGrid
                ...
                gridName="EmployeeAttritionGrid"
              />
            </div>
          </PerfectScrollbar>
        </CardContent>
