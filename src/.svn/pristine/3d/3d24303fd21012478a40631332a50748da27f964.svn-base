...
const ContactBoardList = () => {
  ...
  const [contactBoardId, setContactBoardId] = useState('');
  ...
  useEffect(() => {
    if (contactBoardState.showUpdateForm) {
      router.history.push('/contact-board/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [contactBoardState.showUpdateForm]);
  ...
  const deleteRecord = async () => {
    await dispatch(deleteContactBoard(contactBoardId, session.current_page_permissions.object_id));
    setRefershDataTable(true);
  }

  const showDeleteModal = (id) => {
    setContactBoardId(id)
  ...
  const updateRecord = (id) => {
    dispatch(getContactBoardById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getContactBoardById(id, 'view'))
  }
