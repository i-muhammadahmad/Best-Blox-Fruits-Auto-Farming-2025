...
const RolesList = () => {
  ...
  useEffect(() => {
    if (rolesState.showViewPage) {
      router.history.push('/roles/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [rolesState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteRoles(rolesId, session.current_page_permissions.object_id));
  ...
  const updateRecord = (id) => {
    dispatch(getRolesById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getRolesById(id, 'view'))
  }
