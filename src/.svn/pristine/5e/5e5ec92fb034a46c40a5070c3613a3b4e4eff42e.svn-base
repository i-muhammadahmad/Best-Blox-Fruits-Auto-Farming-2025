...
import {
  setupFloorPlanServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  const handleSetupFloorPlanSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['image_icon'] = getFlagAvator(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(setupFloorPlanServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleSetupFloorPlanSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "setupFloorPlan/getAllServerSide", {
    method: 'POST',
    headers: {
