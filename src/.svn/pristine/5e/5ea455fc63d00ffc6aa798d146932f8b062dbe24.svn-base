...
const AssetApprovalsList = () => {
  ...
  const deleteRecord = async (notification_ids) => {
    await dispatch(rejectAssetApprovals(notification_ids, session.current_page_permissions.object_id));
  ...
  const showDeleteModel = async (id) => {
    ...
    await setRejectionModelDetails({
			...
			submitCallback: () => { deleteRecord([id]) },
      onModelClose: () => { /*nothing to do*/ },
  ...
  const showRejectSelectedModel = async () => {
    await setRejectionModelDetails({
			...
			submitCallback: () => { deleteRecord(selectedNotifications) },
      onModelClose: () => { /*nothing to do*/ },
  ...
  const showRejectAllModel = async () => {
    await setRejectionModelDetails({
			...
			submitCallback: () => { deleteRecord(['all']) },
      onModelClose: () => { /*nothing to do*/ },
        ...
        { (value.is_approved == '0' && value.is_rejected == '0')? 
          <>
            <StyledFab
              ...
              aria-label="Edit"
              size="small"
              onClick={()=> showAssetsApprovalModel(value.notification_id)}
            >
            ...
            <StyledFab
              ...
              aria-label="edit"
              size="small"
              onClick={() => showDeleteModel(value.notification_id)}
            >
