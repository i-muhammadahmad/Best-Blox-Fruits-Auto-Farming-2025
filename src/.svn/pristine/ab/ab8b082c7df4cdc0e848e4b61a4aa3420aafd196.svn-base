...
const CourseCategoryList = () => {
  ...
  useEffect(() => {
    if (courseCategoryState.showViewPage) {
      router.history.push('/course-category/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [courseCategoryState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteCourseCategory(courseCategoryId, session.current_page_permissions.object_id));
  ...
  const updateRecord = (id) => {
    dispatch(getCourseCategoryById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getCourseCategoryById(id, 'view'))
  }
