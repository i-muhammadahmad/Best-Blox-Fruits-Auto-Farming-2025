...
const Results = (props) => {
  ...
  const handleBreakTypesSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(breakTypesServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleBreakTypesSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "break_types/getAllServerSide", {
    method: 'POST',
    headers: {
