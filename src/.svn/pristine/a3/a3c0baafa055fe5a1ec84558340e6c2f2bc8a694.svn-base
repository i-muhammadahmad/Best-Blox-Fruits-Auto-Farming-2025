...
import {
  documentsCategoryServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = (props) => {
  ...
  //handle custom response for Documents Category
  const handleDocumentsCategorySuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(documentsCategoryServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleDocumentsCategorySuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "documents_category/getAllServerSide", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              <CustomDataGrid
                ...
                gridName="DocumentsCategoryGrid"
              />
            </div>
          </PerfectScrollbar>
        </CardContent>
