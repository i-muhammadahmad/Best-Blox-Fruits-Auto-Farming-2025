...
const Results = props => {
  ...
  const handleAuditFormSuccessResponse = (responseObject) => {
    if (
  ...
  const call_backs = {
    ...
    'customResponse': (responseObject) => { return handleAuditFormSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "audit_form/getAllServerSide", {
    method: 'POST',
    headers: {
