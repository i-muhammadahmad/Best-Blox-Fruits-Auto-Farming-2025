...
import {
  clientInvoicesServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError,
  showClientInvoiceAddForm
} from 'actions';
...
const Results = props => {
  ...
  const client_invoice_per = filter(session.user_permissions_raw, function (item) {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(clientInvoicesServerListSuccess()); },
  ...
  const server_side_request = new Request(API_URL + "client_invoices/getAllServerSide", {
    method: 'POST',
    headers: {
  ...
  const handleAddBtnClick = () => {
    setFormAction('Add');
    dispatch(showClientInvoiceAddForm());
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Grid item> 
          {(AccessRights(client_invoice_per[0], 'add') && isEmpty(clientInvoicesState.curPeriodInvoice) ) ?
            <StyledButton
              color="bsuccess"
              variant="contained"
