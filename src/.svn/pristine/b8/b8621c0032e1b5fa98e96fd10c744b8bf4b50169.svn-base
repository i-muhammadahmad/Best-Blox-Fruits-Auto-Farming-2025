...
const Results = props => {
  ...
  const handleUserSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['user_status'] = getUserStatus(value);
        }
        catch (e) {
          console.error(e);
        }
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(usersServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleUserSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "user/getAllServerSide", {
    method: 'POST',
    headers: {
