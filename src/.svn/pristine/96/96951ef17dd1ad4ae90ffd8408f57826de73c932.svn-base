...
const Results = props => {
  ...
  const handleDepartmentSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(departmentsServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleDepartmentSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "departments/getAllServerSide", {
    method: 'POST',
    headers: {
