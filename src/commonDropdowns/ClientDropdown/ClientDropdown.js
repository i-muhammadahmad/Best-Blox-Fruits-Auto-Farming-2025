...
const ClientDropdown = props => {
  const { officesIds, employeesIds, ClientValue, setClientValue, selectedId, selectedChangingId, clientOnChange, id, name, size, renderInput, showSelectAllOption, setAllCampIds, ...attr } = props;
  ...
  useEffect(() => {
    ...
    dispatch(clientDropdownListFetch(offices_ids, emp_ids, session.current_page_permissions.object_id));
    ...
    if (clientState.clientDropdownList) {
      ...
      if (!isEmpty(sCId)) {
        if (isArray(sCId)) {
          ...
          let selected_client = find(clientState.clientDropdownList, ['id', sCId]);
          if(!isEmpty(selected_client)){
            setClientValue(selected_client);
          }
          else{
            setClientValue(null);
          }
