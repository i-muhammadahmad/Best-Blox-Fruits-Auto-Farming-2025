...
import {
  leaveScheduleServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  const { className, refershDataTable, setRefershDataTable, actionsCol, leaveStatusCol, ...rest } = props;
  const dispatch = useDispatch();

  //handle custom response for quizes
  const handleLeaveSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['leave_status'] = leaveStatusCol(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(leaveScheduleServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleLeaveSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "leaves/getMyRequesedLeavesServerSide", {
    method: 'POST',
    headers: {
