...
import {
  mendatoryCoursesServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  const { className, refershDataTable, setRefershDataTable, actionsCol, getQuizStatus, ...rest } = props;
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['publish_status'] = getQuizStatus(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(mendatoryCoursesServerListSuccess()); },
  ...
  const server_side_request = new Request(API_URL + "compliance/mandatoryCourses", {
    method: 'POST',
    headers: {
    ...
    >
      ...
      <Card>
        <CardHeader
          ...
          title="All Compliance Mandatory Courses"
        />
        <Divider />
