...
import {
  transcribeConfigServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  const { className, refershDataTable, setRefershDataTable, actionsCol, ...rest } = props;
  const dispatch = useDispatch();

  const call_backs = {
    'serverSideSuccess': () => { dispatch(transcribeConfigServerListSuccess()); },
  ...
  const server_side_request = new Request(API_URL + "transcribe_config/getAllServerSide", {
    method: 'POST',
    headers: {
