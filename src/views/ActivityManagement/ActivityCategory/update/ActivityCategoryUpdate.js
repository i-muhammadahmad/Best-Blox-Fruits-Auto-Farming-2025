...
const ActivityCategoryUpdate = () => {
  ...
  const [selectedClientId, setSelectedClientId] = useState(null);
	...
	useEffect(() => {
		let record = activityCategoryState.activityCategoryRecord;
		setFormState(formState => ({
			...formState,
			values: {
				...formState.values,
				description: record.description,
        client_id: !isEmpty(record.parent_category) ? record.parent_category.client_id : record.client_id,
		...
		const item = find(activityCategoryState.activityCategoryParentsList, [
			'id',
			record.parent_id
		]);
		setParentValue(item);
		// eslint-disable-next-line react-hooks/exhaustive-deps
    if(!isEmpty(record.parent_category)){
      setSelectedClientId(record.parent_category.client_id);
    }
    else{
  ...
  const changeClientOnParentChange = (newValue) => {
	...
	const handleSubmit = async event => {
		...
		dispatch(updateActivityCategory(formState.values));
	};
	...
	return (
		<Page className={classes.root} title="Update Activity Category">
			<Header />
			<Card className={classes.projectDetails}>
				<CardHeader title="Update Activity Category" />
				<CardContent>
                ...
                <Grid item xs={6} sm={4}>
                    ...
                    disabled={!isEmpty(selectedClientId) && !isEmpty(formState.values.parent_id)}
										renderInput={params => (
											<TextField
												{...params}
												label="Select Client"
						...
						<div className={classes.formGroup}>
							<CKEditor
								...
								data={
									activityCategoryState.activityCategoryRecord.description || ''
								}
...
export default ActivityCategoryUpdate;
