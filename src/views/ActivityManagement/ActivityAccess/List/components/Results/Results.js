...
import {
  activityAccessServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  const handleActivityAccessSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(activityAccessServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleActivityAccessSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "activity_access/getAllServerSide", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              <CustomDataGrid
                ...
                gridName="ActiivityActivity AccessGrid"
              />
            </div>
          </PerfectScrollbar>
        </CardContent>
