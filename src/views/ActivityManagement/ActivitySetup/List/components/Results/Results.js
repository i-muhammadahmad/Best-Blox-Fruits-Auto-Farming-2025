...
import {
  activitySetupServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  const handleActivitySetupSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['unit_processing_time'] = getUnitProcessingTimeHTML(value);
          responseObject.payload[key]['passing_score'] = getPassingScoreHTML(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(activitySetupServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleActivitySetupSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "activity_setup/getAllServerSide", {
    method: 'POST',
    headers: {
