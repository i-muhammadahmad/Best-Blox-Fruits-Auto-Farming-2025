...
import {
  activityCategoryStatusServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  const { className, actionsCol, refershDataTable, setRefershDataTable, ...rest } = props;

  const dispatch = useDispatch();

  const call_backs = {
    'serverSideSuccess': () => { dispatch(activityCategoryStatusServerListSuccess()); },
  ...
  const server_side_request = new Request(API_URL + "activity_categories_status/getAllServerSide", {
    method: 'POST',
    headers: {
    ...
    >
      ...
      <Card>
        <CardHeader
          ...
          title="All Activity Category Status"
        />
        <Divider />
