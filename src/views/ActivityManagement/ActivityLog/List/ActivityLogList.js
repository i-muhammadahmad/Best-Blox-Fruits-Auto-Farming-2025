...
const ActivityLogList = () => {
  ...
  useEffect(() => {
    if (activityLogState.showUpdateForm) {
      router.history.push('/activity-log/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activityLogState.showUpdateForm]);

  useEffect(() => {
    if (activityLogState.showViewPage) {
      router.history.push('/activity-log/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activityLogState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteActivityLog(activityLogId, session.current_page_permissions.object_id));
  ...
  const updateRecord = (id) => {
    dispatch(getActivityLogById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getActivityLogById(id, 'view'))
  }

  const getActivityUnitsLoggedHTML = value => {
    return (
      <p style={{ textAlign: 'left' }} >{value.activity_units_logged}</p>
    );
  }  
  ...
  return (
    ...
    >
      ...
      <Results
        ...
        getActivityUnitsLoggedHTML={getActivityUnitsLoggedHTML}
        extraFiltersState={extraFiltersState}
        setExtraFiltersState={setExtraFiltersState}
        getDeletedStatus={getDeletedStatus}
      />
      <DeleteAlert
        title="Activity Log Delete"
        alertText="Are you sure, You want delete this Activity Log?"
