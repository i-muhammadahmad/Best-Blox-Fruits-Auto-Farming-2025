...
const Results = props => {
  ...
  const handleActivityLogSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['activity_units_logged'] = getActivityUnitsLoggedHTML(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(activityLogServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleActivityLogSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "activity_log/getAllServerSide", {
    method: 'POST',
    headers: {
