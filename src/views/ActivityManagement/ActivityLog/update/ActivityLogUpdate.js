...
const ActivityLogUpdate = () => {
  ...
  const [ActivityCategoryValue, setActivityCategoryValue] = useState(null);
	...
	useEffect(() => {
		...
		each(record, (val, key) => {
			if (
				some(
					['id', 'category_id', 'name', 'activity_id', 'activity_units_logged', 'chk_manual_time', 'manual_time', 'description'],
					el => includes(key, el)
				)
			) {
				vals[key] = val;
    ...
    setSelectedCategoryId(isEmpty(record.category_id) ? 'null' : record.category_id);
		setShowManualTime((record.chk_manual_time == 1) ? true : false);
  ...
  useEffect(() => {
	...
	useEffect(() => {
		if (formState.values.chk_manual_time === true || formState.values.chk_manual_time == 1) {
			setShowManualTime(true);
			setSchema(schema => ({
				...schema,
				manual_time: {
					presence: { allowEmpty: false, message: '^Please Enter Manual Time.' },
					format: {
						pattern: "^([0-9]+)?:?([0-9]+)?$", message: "^Please Enter Valid Time."
					}
				}
			}));
		} else {
			...
			delete schema_obj.manual_time;
			setSchema(schema_obj);
	...
	useEffect(() => {
		if (
			!isEmpty(activitySetupState.activitySetupByRoleList) &&
			!isEmpty(activitySetupState.availableExtraFeilds)
		) {
			let record = activityLogState.activityLogRecord;
			const item = find(activitySetupState.activitySetupByRoleList, [
				'id',
				record.activity_id
			]);
	...
	useEffect(() => {
		if (!activityLogState.showUpdateForm && !activityLogState.showViewPage) {
			router.history.push('/activity-log');
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [activityLogState.showUpdateForm, activityLogState.showViewPage]);

	useEffect(() => {
		dispatch(
			activitySetupByRoleListFetch(session.current_page_permissions.object_id)
		);
  ...
  const activityCategoryOnChange = (event, newValue) => {
	...
	const setCategoryId = category_id => {
		...
		dispatch(hideActivityLogValidationError('category_id'));
	};
	...
	return (
		<Page className={classes.root} title="Update Activity Log">
			<Header />
							...
							<Grid container spacing={3}>
								<Grid item xs={6} sm={3}>
									<ActivityCategoryDropdown
										...
										// showNone={false}
										// clientIds={session.user.employee.client_id}
                    selectedId={selectedCategoryId}
											...
											<Autocomplete
												...
												renderInput={params => (
													<TextField
														...
														variant="outlined"
														error={hasError('activity_id')}
														helperText={
															hasError('activity_id')
																? formState.errors.activity_id[0]
																: null
														}
													/>
												)}
											...
											{!isEmpty(ActivityValue) ? (
												<FormControl>
													<FormHelperText id="component-error-text">
														Unit Time:
														{'[' +
															ActivityValue.unit_processing_time +
															' sec => ' +
															(ActivityValue.unit_processing_time / 60).toFixed(
																2
															) +
															' min]'}
													</FormHelperText>
								...
								<Grid item xs={6} sm={3}>
									<TextField
										...
										fullWidth
										helperText={
											hasError('activity_units_logged')
												? formState.errors.activity_units_logged[0]
												: null
										}
								...
								<Grid item xs={6} sm={3}>
									<FormControlLabel
										control={
											<Checkbox
												checked={formState.values.chk_manual_time || false}
												onChange={handleChange}
												name="chk_manual_time"
											/>
										}
									...
									{showManualTime ? (
										<>
											<TextField
											error={hasError('manual_time')}
											fullWidth
											helperText={
												hasError('manual_time')
													? formState.errors.manual_time[0]
													: null
											}
											label="Manual Time"
											name="manual_time"
											onChange={handleChange}
											value={formState.values.manual_time || ''}
											variant="outlined"
											InputLabelProps={{
												shrink: true,
											}}
							...
							{!isEmpty(addedExtraFeilds) ? (
								<Paper
									elevation={3}
									variant="outlined"
									style={{ marginTtop: '10px', padding: '10px' }}>
