...
import {
  ticketsCategoryServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = (props) => {
  ...
  //handle custom response for tickets category
  const handleTicketsCatgeorySuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(ticketsCategoryServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleTicketsCatgeorySuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "tickets_category/getAllServerSide", {
    method: 'POST',
    headers: {
