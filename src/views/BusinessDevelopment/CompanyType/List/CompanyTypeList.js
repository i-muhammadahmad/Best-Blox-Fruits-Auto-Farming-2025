...
const CompanyTypeList = () => {
  ...
  useEffect(() => {
    if (companyTypeState.showUpdateForm) {
      router.history.push('/company-type/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [companyTypeState.showUpdateForm]);
  ...
  const deleteRecord = async () => {
    await dispatch(deleteCompanyType(companyTypeId, session.current_page_permissions.object_id));
  ...
  const updateRecord = (id) => {
    dispatch(getCompanyTypeById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getCompanyTypeById(id, 'view'))
  }
