...
const EmployeeAttritionAdd = () => {
  ...
  useEffect(() => {
    if(!isEmpty(employeesState.employeeAssetsList) && submitBtnClicked){
  ...
  const showAssetsListPopup = async event => {
    ...
    await dispatch(getEmployeeAllAssets(formState.values.employee_id, session.current_page_permissions.object_id));
  }
