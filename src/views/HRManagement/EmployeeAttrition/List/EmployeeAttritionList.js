...
import {
  deleteEmployeeAttrition,
  getEmployeeAttritionById,
} from 'actions';
...
const EmployeeAttritionList = () => {
  ...
  const [employeeAttritionId, setEmployeeAttritionId] = useState('');
  ...
  useEffect(() => {
    if (employeeAttritionState.showUpdateForm) {
      router.history.push('/employee-attrition/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [employeeAttritionState.showUpdateForm]);

  useEffect(() => {
    if (employeeAttritionState.showViewPage) {
      router.history.push('/employee-attrition/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [employeeAttritionState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteEmployeeAttrition(employeeAttritionId, session.current_page_permissions.object_id));
    setRefershDataTable(true);
  }

  const showDeleteModal = (id) => {
    setEmployeeAttritionId(id)
    setOpenDeleteModel(true)
  }

  const hideDeleteModel = () => {
    setEmployeeAttritionId('')
  }

  const updateRecord = (id) => {
    dispatch(getEmployeeAttritionById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getEmployeeAttritionById(id, 'view'))
  }
  ...
  return (
    ...
    >
      ...
      />
      <DeleteAlert
        title="Employee Attrition Delete"
        alertText="Are you sure, You want delete this Employee Attrition?"
...
export default EmployeeAttritionList;
