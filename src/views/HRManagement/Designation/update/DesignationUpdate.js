...
const DesignationUpdate = () => {
  ...
  useEffect(() => {
    let record = designationState.designationRecord;
    setFormState(formState => ({
      ...formState,
      values: {
        ...formState.values,
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [designationState.designationRecord])

  useEffect(() => {
    let record = designationState.designationRecord
    const item = find(designationCategoryState.designationCategoryList, ['id', record.category_id])
    setCategoryValue(item);
    if(!isEmpty(item)){
      setDesignationLevel(item.level);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleSubmit = async event => {
    ...
    dispatch(updateDesignation(formState.values));
  }
  ...
  return (
    ...
    >
      ...
      >
        <CardHeader title="Update Designation" />
        <CardContent>
          <form
          ...
          >
            ...
            <div className={classes.formGroup}>
              <CKEditor
                ...
                data={designationState.designationRecord.description || ''}
