...
const EmployeeAttritionReasonList = () => {
  ...
  const [employeeAttritionReasonId, setEmployeeAttritionReasonId] = useState('');
  ...
  useEffect(() => {
    if (employeeAttritionReasonState.showUpdateForm) {
      router.history.push('/attrition-reasons/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [employeeAttritionReasonState.showUpdateForm]);
  ...
  const deleteRecord = async () => {
    await dispatch(deleteEmployeeAttritionReason(employeeAttritionReasonId, session.current_page_permissions.object_id));
    setRefershDataTable(true);
  }

  const showDeleteModal = (id) => {
    setEmployeeAttritionReasonId(id)
    setOpenDeleteModel(true)
  }

  const hideDeleteModel = () => {
    setEmployeeAttritionReasonId('')
  }

  const updateRecord = (id) => {
    dispatch(getEmployeeAttritionReasonById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getEmployeeAttritionReasonById(id, 'view'))
  }
  ...
  return (
    <Page
      ...
      title="Employee Attrition Reason List"
    >
      <Header />
      <Results
...
export default EmployeeAttritionReasonList;
