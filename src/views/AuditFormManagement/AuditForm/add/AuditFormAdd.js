...
const AuditFormAdd = () => {
	...
	// const [selectedAuditFormSetupId, setSelectedAuditFormSetupId] = useState(null);
	...
	const [schema, setSchema] = useState({
		reference: {
			presence: {
				allowEmpty: false,
				message: '^Transaction Refrence is required'
			}
		},
		employee_id: {
	...
	useEffect(() => {
		...
		if (!isEmpty(auditFormSetupState.auditFormSetupCheckpointsList)) {
			forEach(auditFormSetupState.auditFormSetupCheckpointsList, function(
				value,
				key
			) {
				let cid = value.id;
