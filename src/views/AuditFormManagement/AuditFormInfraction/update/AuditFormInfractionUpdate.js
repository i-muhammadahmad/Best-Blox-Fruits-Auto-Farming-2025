...
import { isEmpty, find, forEach, map } from 'lodash';
  ...
  useEffect(() => {
    ...
    setFormState(formState => ({
      ...
      values: {
        ...
        'deduction_percentage': record.deduction_percentage,
        'id': record.id
      },
      touched: {
        ...formState.touched,
    ...
    if(record.is_all === 1){
      let iclient = [];
      iclient.push('all');
      setClientIds(iclient);
    }
    else{
      if (!isEmpty(record.infraction_clients)) {
        let iclient = [];
        forEach(record.infraction_clients, function (value, key) {
          iclient.push(value.client_id);
        });
        setClientIds(iclient);
      }
    }
              ...
              <Grid container spacing={3}>
                <Grid item xs={6} sm={4}>
                  <AuditFormInfractionCategoryDropdown
                    ...
                    selectedId={auditFormInfractionState.auditFormInfractionRecord.category_id}
                    id="category_id"
                    name="category_id"
