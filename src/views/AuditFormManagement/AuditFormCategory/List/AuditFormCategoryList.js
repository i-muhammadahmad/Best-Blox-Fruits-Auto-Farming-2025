...
const AuditFormCategoryList = () => {
  ...
  const [auditFormCategoryId, setAuditFormCategoryId] = useState('');
  ...
  useEffect(() => {
    if (auditFormCategoryState.showViewPage) {
      router.history.push('/audit-form-category/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [auditFormCategoryState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteAuditFormCategory(auditFormCategoryId, session.current_page_permissions.object_id));
    setRefershDataTable(true);
  }

  const showDeleteModal = (id) => {
    setAuditFormCategoryId(id)
    setOpenDeleteModel(true)
  }

  const hideDeleteModel = () => {
    setAuditFormCategoryId('')
  }

  const updateRecord = (id) => {
    dispatch(getAuditFormCategoryById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getAuditFormCategoryById(id, 'view'))
  }

  const getActions = (value) => {
    return (
      <div className={'actionClass'} style={{ textAlign: 'center', whiteSpace: 'nowrap' }}>
        {console.log(session.current_page_permissions)}
  ...
  return (
    <Page
      ...
      title="Audit Form Category List"
    >
      <Header />
      <Results
...
export default AuditFormCategoryList;
