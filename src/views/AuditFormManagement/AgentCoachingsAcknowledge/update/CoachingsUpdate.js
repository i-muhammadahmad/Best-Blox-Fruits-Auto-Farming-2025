...
const CoachingsUpdate = () => {
	...
	const [updateCoachingModelOpen, setUpdateCoachingModelOpen] = useState(false);

	const completeCoaching = async (id, audit_form_id, coachingVal, by) => {
		await dispatch(updateCoaching(id, coachingVal, by, session.current_page_permissions.object_id));
		await setUpdateCoachingModelOpen(false);
		dispatch(getAuditFormById(audit_form_id, 'update'))
	};
	...
	useEffect(() => {
		if (!auditFormState.showViewPage && !auditFormState.showUpdateForm) {
			router.history.push('/agent-coachings-acknowledge');
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const auditFormCategoryOnChange = (event, newValue) => {
	...
	return (
		<Page className={classes.root} title="Update Coachings">
			<Header />
			<Card className={classes.projectDetails}>
				<CardHeader title="Update Coachings" />
				<CardContent>
...
export default CoachingsUpdate;
