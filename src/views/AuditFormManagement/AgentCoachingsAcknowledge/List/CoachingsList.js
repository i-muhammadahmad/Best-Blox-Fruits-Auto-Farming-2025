...
const CoachingsList = () => {
  ...
  const [extraFiltersState, setExtraFiltersState] = useState({
    isValid: false,
    values: {employee_id: session.user.employee_id},
    touched: {},
    errors: {}
  });
  ...
  useEffect(() => {
    if (auditFormState.showUpdateForm) {
      router.history.push('/agent-coachings-acknowledge/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (auditFormState.showViewPage) {
      router.history.push('/agent-coachings-acknowledge/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
