...
import {
  approvalProfilesServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  const handleAprovalProfileSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(approvalProfilesServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleAprovalProfileSuccessResponse(responseObject) }
  }


  const server_side_request = new Request(API_URL + "approval_profiles/getAllServerSide", {
    method: 'POST',
    headers: {
