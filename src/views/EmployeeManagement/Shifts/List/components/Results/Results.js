...
const Results = props => {
  ...
  const handleShiftsSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(shiftsServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleShiftsSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "shifts/getAllServerSide", {
    method: 'POST',
    headers: {
