...
const EmployeesUpdateForm = (props) => {
  ...
  useEffect(() => {
    ...
    setSelectedOfficeId([record.office_id]);
    setSelectedClientId([record.client_id]);

    //papolating secondary clients
    if (!isEmpty(record.employee_clients)) {
      let sclient = [];
      forEach(record.employee_clients, function (value, key) {
        if (value.is_primary == false) {
          sclient.push(value.client_id);
        }
      });
      setSecondaryClientsIds(sclient);
    }
  ...
  useEffect(() => {
    ...
    dispatch(workstationsByOfficesListFetch(formState.values.office_id, session.current_page_permissions.object_id));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formState.values.office_id]);

  useEffect(() => {
    let record = employeesState.employeesRecord;
    const item = find(workstationsState.workstationsByOfficeList, ['id', record.workstation_id]);
  ...
  const setOfficeId = office_id => {
    ...
    dispatch(workstationsByOfficesListFetch(office_id, session.current_page_permissions.object_id));
                ...
                <Grid container spacing={3}>
                  ...
                  <Grid item xs={12} sm={4}>
                    <ClientDropdown
                      ...
                      selectedId={formState.values.camp_ids}
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} sm={12}>
                    ...
                    <CKEditor
                      ...
                      data={employeesState.employeesRecord.description || ''}
                      ...
                      <ListItemIcon>
                        <div className={classes.thumb} >
                          <div className={classes.thumbInner}>
                            <img
                              src={API_URL+employeesState.employeesRecord.profile_pic}
