...
const Results = props => {
  ...
  const handleShiftBreakSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(shiftBreaksServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleShiftBreakSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "shift_breaks/getAllServerSide", {
    method: 'POST',
    headers: {
