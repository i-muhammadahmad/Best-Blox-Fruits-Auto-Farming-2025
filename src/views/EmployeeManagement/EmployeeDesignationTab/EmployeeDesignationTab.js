...
const EmployeeDesignationTab = (props) => {
  ...
  useEffect(() => {
    dispatch(designationsDropdownListFetch(session.current_page_permissions.object_id));
  }, []);

  useEffect(() => {
    
    dispatch(departmentsDropdownListFetch([formState.values.office_id], session.current_page_permissions.object_id));
    // eslint-disable-next-line react-hooks/exhaustive-deps
