...
const EmployeeShifts = props => {
  ...
  useEffect(() => {
    if (
      ...
      dispatch(getShiftsByClient(employeesState.employeesRecord.client_id, session.current_page_permissions.object_id))
    }
  }, []);

  useEffect(() => {
    if (!isEmpty(employeesState.employeesRecord)) {
      dispatch(employeeShiftsListFetch(employeesState.employeesRecord.id, session.current_page_permissions.object_id));
    }
  }, []);
