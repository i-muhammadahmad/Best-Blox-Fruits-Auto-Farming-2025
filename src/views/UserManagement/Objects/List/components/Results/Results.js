...
const Results = props => {
  ...
  const handleObjectSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(objectsServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleObjectSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "objects/getAllServerSide", {
    method: 'POST',
    headers: {
