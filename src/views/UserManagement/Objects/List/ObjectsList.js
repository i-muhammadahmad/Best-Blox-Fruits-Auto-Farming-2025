...
const ObjectsList = () => {
  ...
  useEffect(() => {
    if (objectsState.showViewPage) {
      router.history.push('/objects/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [objectsState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteObjects(objectsId, session.current_page_permissions.object_id));
  ...
  const updateRecord = (id) => {
    dispatch(getObjectsById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getObjectsById(id, 'view'))
  }
