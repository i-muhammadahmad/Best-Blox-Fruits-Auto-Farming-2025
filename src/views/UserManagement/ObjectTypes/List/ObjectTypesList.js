...
const ObjectTypesList = () => {
  ...
  useEffect(() => {
    if (objectTypesState.showUpdateForm) {
      router.history.push('/object-types/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [objectTypesState.showUpdateForm]);

  useEffect(() => {
    if (objectTypesState.showViewPage) {
      router.history.push('/object-types/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [objectTypesState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteObjectTypes(objectTypesId, session.current_page_permissions.object_id));
  ...
  const updateRecord = (id) => {
    dispatch(getObjectTypesById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getObjectTypesById(id, 'view'))
  }
  ...
  return (
    ...
    >
      ...
      />
      <DeleteAlert
        title="Object Types Delete"
        alertText="Are you sure, You want delete this Object Types?"
