...
const Results = props => {
  ...
  const handleObjectTypeSetupSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(objectTypesServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleObjectTypeSetupSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "object_types/getAllServerSide", {
    method: 'POST',
    headers: {
