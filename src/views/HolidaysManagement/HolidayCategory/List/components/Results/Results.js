...
import {
  holidayCategoryServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  const handleHolidayCategorySuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(holidayCategoryServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleHolidayCategorySuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "holiday_categories/getAllServerSide", {
    method: 'POST',
    headers: {
