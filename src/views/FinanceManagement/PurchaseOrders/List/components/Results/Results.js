...
import {
  purchaseOrdersServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError,
...
const Results = props => {
  ...
  //handle custom response for purchase orders
  const handlePurchaseOrdersSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(purchaseOrdersServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handlePurchaseOrdersSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "purchaseOrders/getAllServerSide", {
    method: 'POST',
    headers: {
