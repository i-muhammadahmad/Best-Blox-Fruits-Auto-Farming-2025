...
const Results = props => {
  ...
  //handle custom response for vendors
  const handleVendorsSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(vendorsServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleVendorsSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "vendor/getAllServerSide", {
    method: 'POST',
    headers: {
