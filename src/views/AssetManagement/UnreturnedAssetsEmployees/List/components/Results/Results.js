...
const Results = props => {
  ...
  const handleEmployeeSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['profile_pic_icon'] = getProfilePicAvator(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(employeesListServerSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleEmployeeSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "employees/getUnreturnedAssetsEmployees", {
    method: 'POST',
    headers: {
  ...
  return (
    ...
    >
      ...
      <Card>
        <CardHeader
          title={
            <>All Unreturned Assets Employees
