...
const AssetsDiscard = () => {
  ...
  const [openDiscardModel, setOpenDiscardModel] = React.useState(false);
  ...
  useEffect(() => {
    ...
    setFormState(formState => ({
      ...
      values: {
        ...formState.values,
        'id': record.id,
        'discard_notes': record.discard_notes,
      },
      touched: {
        ...formState.touched,
        'id': true,
  ...
  const setDiscardNotes = discard_notes => {
    setFormState(formState => ({
      ...formState,
      values: {
        ...formState.values,
  ...
  const showDiscardModel = () => {
    setOpenDiscardModel(true)
  }

  const hideDiscardModel = () => {
    setOpenDiscardModel(false)
  }

  const discardAsset = () => {
    dispatch(saveAssetsDiscarding(formState.values));
    setOpenDiscardModel(false)
  }
    ...
    >
      <CardContent>
        ...
        >
          ...
          <div className={classes.formGroup}>
            <CKEditor
              ...
              data={assetsState.assetsRecord.discard_notes || ''}
              onChange={(event, editor) => {
                const data = editor.getData();
                setDiscardNotes(data)
              }}
            />
            <FormControl error={hasError('discard_notes')} >
              <FormHelperText id="component-error-text">{hasError('discard_notes') ? formState.errors.discard_notes[0] : null}</FormHelperText>
            </FormControl>
          </div>
          <StyledButton
            color="bprimary"
        ...
        <Dialog
          ...
          onClose={hideDiscardModel}
          aria-labelledby="alert-dialog-title"
          aria-describedby="alert-dialog-description"
        >
          <DialogTitle id="alert-dialog-title">Discard Asset</DialogTitle>
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              Are you sure you want to discard this assets? <br />
              You will not be able to edit a discard asset.
            </DialogContentText>
          </DialogContent>
          <DialogActions>
            <StyledButton onClick={hideDiscardModel}   >
              Cancel
            </StyledButton>
            <StyledButton
              variant="contained"
