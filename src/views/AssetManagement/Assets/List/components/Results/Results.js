...
const Results = props => {
  ...
  //handle custom response for assets
  const handleAssetsSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(assetsServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleAssetsSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "assets/getAllServerSide", {
    method: 'POST',
    headers: {
