...
const Results = props => {
  ...
  const handleActivtyTypeSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['atype_icon_avator'] = getAtypeIconAvator(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(assetTypesServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleActivtyTypeSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "asset_types/getAllServerSide", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              ...
              <CustomDataGrid
                columns={Columns}
                dataSource={
                  new ServerSideRequest(server_side_request, '', call_backs, extraFiltersState.values, session.current_page_permissions.object_id)
                }
                gridName="AssetTypesGrid"
              />
            </div>
          </PerfectScrollbar>
        </CardContent>
