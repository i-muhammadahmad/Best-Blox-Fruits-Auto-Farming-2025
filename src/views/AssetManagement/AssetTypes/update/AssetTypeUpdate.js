...
const AssetTypeUpdate = () => {
  ...
  useEffect(() => {
    if(!isEmpty(assetTypesState.assetTypesRecord)){
      let record = assetTypesState.assetTypesRecord;

      //setting binder_with array for validation
      let bindings = formState.values.binder_with;
      if(record.is_binded_with_office){ bindings.push('is_binded_with_office')  }
      if(record.is_binded_with_employee){ bindings.push('is_binded_with_employee')  }
      if(record.is_binded_with_workstation){ bindings.push('is_binded_with_workstation')  }
      ...
      setFormState(formState => ({
        ...
        values: {
          ...
          'id': record.id,
          'name': record.name,
          'is_binded_with_office': record.is_binded_with_office,
          'is_binded_with_employee': record.is_binded_with_employee,
          'is_binded_with_workstation': record.is_binded_with_workstation,
          'approval_profile_type': record.approval_profile_type,
      ...
      dispatch(setAssetTypeId(record.id));
    }
    else{
