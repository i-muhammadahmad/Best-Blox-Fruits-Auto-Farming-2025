...
const ITEmails = (props) => {
  ...
  const [formState, setFormState] = useState({
    ...
    values: {
      ...
      'setting_type': 'OfficesITEmail',
      'settings': {},
      'setting_descriptor': {
        'setting_type': 'This settings is used to save IT Email against Office',
        'setting_key': 'Office Id',
        'setting_val': 'IT Email'
      }
    },
    touched: {
  ...
  useEffect(() => {
    dispatch(getITEmailSettings('OfficesITEmail', session.current_page_permissions.object_id));
    dispatch(officesListFetch(session.current_page_permissions.object_id));
  }, []);

  useEffect(() => {
    const e_settings = {};
    forEach(settingsState.settingsITEmail, function(set, key) {
      ...
      <CardContent>
        ...
        >
          <div className={classes.formGroup}>
            <p style={{marginBottom: '25px'}}>Enter IT Email to recieve email notification whenever a new ticket is created.</p>
