...
const Results = props => {
  ...
  const handleOfficesSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['flag_image_icon'] = getFlagAvator(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(officesServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleOfficesSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "offices/getAllServerSide", {
    method: 'POST',
    headers: {
