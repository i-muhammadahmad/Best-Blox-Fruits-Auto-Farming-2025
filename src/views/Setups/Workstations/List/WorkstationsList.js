...
const WorkstationsList = () => {
  ...
  const [workstationsId, setWorkstationsId] = useState('');
  ...
  useEffect(() => {
    if (workstationsState.showUpdateForm) {
      router.history.push('/workstations/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [workstationsState.showUpdateForm]);
  ...
  const deleteRecord = async () => {
    await dispatch(deleteWorkstations(workstationsId, session.current_page_permissions.object_id));
    setRefershDataTable(true);
  }

  const showDeleteModal = (id) => {
    setWorkstationsId(id)
  ...
  const updateRecord = (id) => {
    dispatch(getWorkstationsById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getWorkstationsById(id, 'view'))
  }
