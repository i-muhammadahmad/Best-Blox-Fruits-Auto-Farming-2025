...
import {
  courseCategoryServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = (props) => {
  ...
  //handle custom response for activity category
  const handleCourseCategorySuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(courseCategoryServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleCourseCategorySuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "course_category/getAllServerSide", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              <CustomDataGrid
                ...
                gridName="ActivityCategoriesGrid"
              />
            </div>
          </PerfectScrollbar>
        </CardContent>
