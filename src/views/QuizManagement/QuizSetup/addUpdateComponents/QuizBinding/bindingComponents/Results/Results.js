...
const Results = props => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(quizBindingServerListSuccess()); },
  ...
  const server_side_request = new Request(API_URL + "quiz_binding/getAllQuizBindingServerSide", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              <CustomDataGrid
                columns={Columns}
                dataSource={
                  new ServerSideRequest(server_side_request, actionsCol, call_backs, extraFilters, session.current_page_permissions.object_id)
                }
                gridName="CourseBindingGrid"
              />
            </div>
          </PerfectScrollbar>
        </CardContent>
