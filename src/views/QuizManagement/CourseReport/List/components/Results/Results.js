...
const Results = props => {
  const { className, refershDataTable, setRefershDataTable, actionsCol, getQuizStatus, getQuizLastScore, paramsObj,  ...rest } = props;
  const dispatch = useDispatch();

  console.log(paramsObj);

  //handle custom response for quizes
  const handleQuizSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['quiz_stauts'] = getQuizStatus(value);
          responseObject.payload[key]['quiz_last_score'] = getQuizLastScore(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(quizServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleQuizSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "course_report/getAllServerSide", {
    method: 'POST',
    headers: {
          ...
          <PerfectScrollbar>
            <div className={classes.inner}>
              <CustomDataGrid
                columns={Columns}
                dataSource={
                  new ServerSideRequest(server_side_request, '', call_backs, paramsObj)
                }
                gridName="CourseGrid"
              />
            </div>
