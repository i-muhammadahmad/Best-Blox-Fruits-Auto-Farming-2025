...
const DocumentsCategoryList = () => {
  ...
  const [documentsCategoryId, setDocumentsCategoryId] = useState('');
  ...
  useEffect(() => {
    if (documentsCategoryState.showViewPage) {
      router.history.push('/documents-category/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [documentsCategoryState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteDocumentsCategory(documentsCategoryId, session.current_page_permissions.object_id));
  ...
  const updateRecord = (id) => {
    dispatch(getDocumentsCategoryById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getDocumentsCategoryById(id, 'view'))
  }
