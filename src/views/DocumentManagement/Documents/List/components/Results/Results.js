...
const Results = props => {
  ...
  const handleDocumentsSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(documentsServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleDocumentsSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "documents/getAllDocumentsServerSide", {
    method: 'POST',
    headers: {
