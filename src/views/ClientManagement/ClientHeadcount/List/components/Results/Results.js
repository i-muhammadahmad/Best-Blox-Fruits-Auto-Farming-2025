...
import {
  clientHeadcountServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  //handle custom response for client headcount
  const handleClientHeadcountSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(clientHeadcountServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleClientHeadcountSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "client_headcount/getAllServerSide", {
    method: 'POST',
    headers: {
