...
const Results = props => {
  ...
  const handleClientSuccessResponse = (responseObject) => {
    ...
    if (
      ...
      forEach(responseObject.payload, function (value, key) {
        ...
        try {
          ...
          responseObject.payload[key]['client_image_icon'] = getClientImageAvator(value);
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(clientServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleClientSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "client/getAllServerSide", {
    method: 'POST',
    headers: {
