...
const ClientUpdate = () => {
  ...
  useEffect(() => {
    if (!clientState.showUpdateForm && !clientState.showViewPage) {
      router.history.push('/client');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [clientState.showUpdateForm, clientState.showViewPage]);
  ...
  useEffect(() => {
    dispatch(clientManagerListFetch(session.current_page_permissions.object_id))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
