...
import {
  emailTemplatesServerListSuccess,
  showCommonLoader,
  hideCommonLoader,
  tokenError,
  showGeneralError
...
const Results = props => {
  ...
  //handle custom response for email templates
  const handleEmailTemplatesSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(emailTemplatesServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleEmailTemplatesSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "email_templates/getAllServerSide", {
    method: 'POST',
    headers: {
