...
const EmailTemplatesList = () => {
  ...
  const [emailTemplatesId, setEmailTemplatesId] = useState('');
  ...
  useEffect(() => {
    if (emailTemplatesState.showUpdateForm) {
      router.history.push('/email-template/update');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [emailTemplatesState.showUpdateForm]);

  useEffect(() => {
    if (emailTemplatesState.showViewPage) {
      router.history.push('/email-template/view');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [emailTemplatesState.showViewPage]);

  const deleteRecord = async () => {
    await dispatch(deleteEmailTemplates(emailTemplatesId, session.current_page_permissions.object_id));
    setRefershDataTable(true);
  }

  const showDeleteModal = (id) => {
    setEmailTemplatesId(id)
  ...
  const updateRecord = (id) => {
    dispatch(getEmailTemplatesById(id, 'update'))
  }

  const viewRecord = (id) => {
    dispatch(getEmailTemplatesById(id, 'view'))
  }
  ...
  return (
    ...
    >
      ...
      <DeleteAlert
        ...
        alertText="Are you sure, You want delete this Email Template?"
