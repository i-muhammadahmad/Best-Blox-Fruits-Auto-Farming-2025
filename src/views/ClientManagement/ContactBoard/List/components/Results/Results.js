...
const Results = props => {
  ...
  //handle custom response for contact board
  const handleConatctBoradSuccessResponse = (responseObject) => {
  ...
  const call_backs = {
    'serverSideSuccess': () => { dispatch(contactBoardServerListSuccess()); },
    ...
    'customResponse': (responseObject) => { return handleConatctBoradSuccessResponse(responseObject) }
  }

  const server_side_request = new Request(API_URL + "contact_board/getAllServerSide", {
    method: 'POST',
    headers: {
